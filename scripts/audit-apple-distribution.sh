...
if [[ "$REQUIRE_COMPLETE" == "1" ]]; then
  ...
  run_step "Check App Store Connect identity strictly" ./scripts/check-app-store-identity.sh --require-ready
  ...
  run_step "Check App Store Connect identity" ./scripts/check-app-store-identity.sh
  ...
  run_step "Check publication/legal gates" ./scripts/check-publication-legal-gates.sh
  run_step "Check release recovery readiness" ./scripts/check-release-recovery-readiness.sh
fi
