...
| Run consolidated completion audit | `./scripts/audit-apple-distribution.sh --require-complete` | Script path exists and runs the repo, CI, App Store metadata, secret, Developer ID, MAS upload, and human gate checks in one safe non-uploading audit. Strict mode fails until Apple credentials and approvals exist. | Prepared |
