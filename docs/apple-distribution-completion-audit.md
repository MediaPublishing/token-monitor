...
| Keep release operations repeatable | `.github/workflows/release.yml`, `scripts/package-release.sh`, `scripts/preflight-release.sh`, `scripts/verify-public-release.sh` | CI covers release script smoke checks; the release workflow uses the Apple access handoff gate, the package-level strict distribution gate, blocks signed non-notarized releases, and treats unsigned artifacts as preview-only. | Prepared |
