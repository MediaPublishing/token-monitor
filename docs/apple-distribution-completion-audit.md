...
| Keep issue fixing safe | `.github/ISSUE_TEMPLATE/parser-layout-bug.yml`, `.github/ISSUE_TEMPLATE/install-update-bug.yml`, `docs/issue-triage-runbook.md`, `docs/apple-distribution-readiness.md` | Parser and install/update issue templates warn that GitHub Issues are public and block raw debug dumps, private account data, Apple credentials, certificates, private keys, and GitHub Secrets. The triage runbook documents intake, parser fix flow, install/update checks, hotfix path, and sanitized public replies. | Prepared |
