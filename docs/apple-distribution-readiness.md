...
- Current local direct-DMG build is `1.0.26` build `27` and is ad hoc signed when no `TOKEN_MONITOR_CODESIGN_IDENTITY` is provided.
...
- `scripts/build-mas-app.sh` produces a separate `1.0.26` build `27` MAS candidate.
...
- Current public GitHub/Sparkle preview release: `v1.0.26` / build `27`.
- Public release verification command for this preview line: `./scripts/verify-public-release.sh v1.0.26 1.0.26 27`.
...
- Current GitHub preview release: `https://github.com/MediaPublishing/token-monitor/releases/tag/v1.0.26`.
- Current release workflow evidence: run `gh run list --repo MediaPublishing/token-monitor --workflow Release --limit 1` after publishing and confirm the `v1.0.26` release run passed.
...
- The current `dist/TokenMonitor.app` is version `1.0.26` build `27` and ad hoc signed.
