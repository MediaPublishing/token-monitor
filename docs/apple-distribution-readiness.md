...
- `swift test` passes with 36 tests.
- `scripts/build-app.sh` builds `dist/TokenMonitor.app`.
- Current local direct-DMG build is `1.0.22` build `23` and is ad hoc signed when no `TOKEN_MONITOR_CODESIGN_IDENTITY` is provided.
...
- `scripts/build-mas-app.sh` produces a separate `1.0.22` build `23` MAS candidate.
...
- Current public GitHub/Sparkle preview release: `v1.0.22` / build `23`.
- Public release verification command for this preview line: `./scripts/verify-public-release.sh v1.0.22 1.0.22 23`.
...
- Current GitHub preview release: `https://github.com/MediaPublishing/token-monitor/releases/tag/v1.0.22`.
- Current release workflow evidence: run `gh run list --repo MediaPublishing/token-monitor --workflow Release --limit 1` after publishing and confirm the `v1.0.22` release run passed.
- Current main CI evidence: run `gh run list --repo MediaPublishing/token-monitor --branch main --limit 1` and confirm the latest `main` run passed.
- Release list verification should show `v1.0.0` through the current preview release marked as prerelease until Developer ID signing/notarization is available.
...
- Regression coverage: `swift test` passes with 36 tests.
...
- The current `dist/TokenMonitor.app` is version `1.0.22` build `23` and ad hoc signed.
